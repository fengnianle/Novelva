...
- 德语可分动词word必须是不定式原形(如anfangen)，surface为句中分离形态(如fängt...an)
- 英语词组动词/短语动词word为原形(如look up)，surface为句中形态(如looked it up)`;

export const SENTENCE_PROMPT_FORMAT_SCHEMA = `
{context}

JSON格式返回（无多余文字）：
{"language":"ISO 639-1代码","translation":"中文翻译","grammar_points":[{"point":"名称","explanation":"说明"}],"key_expressions":[{"expression":"搭配","meaning":"含义"}],"words":[{"word":"原形","surface":"句中形态","meaning":"语境含义","pos":"词性"}]}`;
  ...
  loadSettings: async () => {
    try {
      ...
      if (settings) {
        ...
        // Migrate: remove deprecated 'explanation' field from old prompt
        if (sentInstr.includes('explanation:') || sentInstr.includes('explanation：')) sentInstr = DEFAULT_SENTENCE_INSTRUCTION;
        // Migrate: add separable verb / phrasal verb instructions
        if (!sentInstr.includes('可分动词')) sentInstr = DEFAULT_SENTENCE_INSTRUCTION;
        set({
          darkMode: dark,
